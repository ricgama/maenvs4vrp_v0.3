...
class Environment(PEnv):
    """
    PCVRP parallel environment generator class.
    """
    def __init__(self,
    ...
    def _update_state_all(self, actions):
        ...
        # Done agents' time is set to end_time
        self.td_state['agents']['cur_time'] = torch.where(
            agents_done, self.td_state['end_time'].unsqueeze(-1).expand_as(time_update), time_update
        )
    ...
    def _update_solution(self, actions):
        ...
        if 'agents' in self.td_state['solution'].keys():
            self.td_state['solution', 'agents'] = torch.cat(
                [self.td_state['solution', 'agents'], self.td_state['cur_agent_idx']], dim=-1
            )
        else:
