...
# Team Orienteering Problem (TOP) parallel environment
...
class Environment(PEnv):
    ...
    * v0: Initial versions release (0.1.0)
    ...
    def _update_all_agents_feasibility(self):
        ...
        # Distance/time from each agent to each node: [B, A, N]
        ...
        distance2j = torch.norm(locs_exp - coords_exp, dim=-1)   # [B, A, N]
        time2j     = distance2j / self.td_state['speed']          # [B, A, N]
        ...
        arrivej = ptime + time2j   # [B, A, N]

        # c1: agent can serve node and return to depot before end_time
        c1 = (arrivej +
    ...
    def _post_process_mask(self, mask):
        ...
        No force_visit for TOP (profit maximization, visits are optional).
        """
        batch_size = self.td_state.batch_size
    ...
    def _update_state_all(self, actions):
        ...
        time_update = arrivej + service_time  # [B, A]
    ...
    def check_solution_validity(self):
        ...
        for ii in range(sorted_data.size(1)):
            ...
            assert torch.all(
                arrivej + service_time + time2depot <= self.td_state['end_time'].unsqueeze(-1)
        ...
        assert torch.all(
            (visited_nodes_exc_depot == 0) | (visited_nodes_exc_depot == 1)
        ), "Nodes were visited more than once!"
