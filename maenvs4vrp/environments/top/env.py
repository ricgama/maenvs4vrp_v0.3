...
# Team Orienteering Problem (TOP) environment
    ...
    * v0: Initial versions release (0.1.0)
    ...
    def reset_agent_select(self,
        ...
        Args:
            num_agents(int, optional): Total number of agents. Defaults to None.
            num_nodes(int, optional): Total number of nodes. Defaults to None.
            service_times(float, optional): service times for each node. Defaults to None.
    ...
    def reset_observe(self,
              ...
              seed:int|None=None,
              device: Optional[str] = "cpu",
              obs_list: Optional[List[str]] = ['nodes_static', 'all_agents_action_mask']) -> TensorDict:
        """
        Resets the environment and sets the current agent.
    ...
    def reset_agent_select_observe(self,
              ...
              seed:int|None=None,
              device: Optional[str] = "cpu",
              obs_list: Optional[List[str]] = ['nodes_static', 'action_mask', 'agent']) -> TensorDict:
        """
        Resets the environment and sets the current agent.
        ...
        Args:
            num_agents(int, optional): Total number of agents. Defaults to None.
            num_nodes(int, optional): Total number of nodes. Defaults to None.
            service_times(float, optional): Total service times for each node. Defaults to None.
            speed(float, optional): Vehicles' speed. Defaults to None.
            instance_name(str, optional): Instance name. Defaults to None.
            sample_type(str): Sample type. It can be "random", "augment" or "saved". Defaults to "random".
            force_visit(bool): It forces the agent to visit all feasible nodes before going back to depot. Defaults to True.
    ...
    def _update_curr_agent_feasibility(self):
        ...
        service_startj = ptime + time2j

        c1 = service_startj + self.td_state['service_time'] + self.td_state['time2depot'] <= self.td_state['end_time'].unsqueeze(-1)
    ...
    def _update_all_agents_feasibility(self):
        ...
        # Get dimensions directly from tensors
        num_agents = self.td_state['agents']['cur_node_idx'].size(1)  # number of agents

        # Get base mask from active nodes
        # active_nodes_mask: [B, N] -> [B, 1, N] -> [B, num_agents, N]
        active_nodes = self.td_state['nodes']['active_nodes_mask'].unsqueeze(1)  # [B, 1, N]
        active_nodes = active_nodes.expand(-1, num_agents, -1)  # [B, num_agents, N]

        # current agent action masks: [B, num_agents, N]
        curr_agent_masks = self.td_state['agents']['action_mask']  # [B, num_agents, N]

        # Start with intersection of active nodes and current agent masks
        _mask = active_nodes & curr_agent_masks  # Use & for boolean, * also works but & is clearer

        # Max tour duration constraints
        # Get current node coordinates for each agent
        cur_node_idx = self.td_state['agents']['cur_node_idx']  # [B, num_agents]

        # Gather coordinates - need to be careful with dimensions
        # cur_node_idx: [B, num_agents]
        # We need to index coords [B, N, 2] with indices [B, num_agents]
        cur_node_coords = torch.gather(
            self.td_state['coords'],  # [B, N, 2]
            1,
            cur_node_idx.unsqueeze(-1).expand(-1, -1, 2)  # [B, num_agents, 2]
        )  # [B, num_agents, 2]

        # Get current time for each agent
        cur_time = self.td_state['agents']['cur_time']  # [B, num_agents]
        # Calculate distance to all nodes for each agent
        # Expand for broadcasting: cur_coords [B, num_agents, 2] -> [B, num_agents, 1, 2]
        cur_coords_expanded = cur_node_coords.unsqueeze(2)  # [B, num_agents, 1, 2]
        # coords [B, N, 2] -> [B, 1, N, 2]
        all_coords_expanded = self.td_state['coords'].unsqueeze(1)  # [B, 1, N, 2]

        # Compute Euclidean distance
        diff = cur_coords_expanded - all_coords_expanded  # [B, num_agents, N, 2]
        distance2j = torch.sqrt((diff ** 2).sum(dim=-1))  # [B, num_agents, N]
        # Apply digit rounding if specified
        if hasattr(self, 'n_digits') and self.n_digits is not None:
            distance2j = torch.floor(self.n_digits * distance2j) / self.n_digits

        # Calculate travel time
        time2j = distance2j / self.td_state['speed'].unsqueeze(-1)  # [B, num_agents, N]
        if hasattr(self, 'n_digits') and self.n_digits is not None:
            time2j = torch.floor(self.n_digits * time2j) / self.n_digits

        # Calculate service start time for each agent and node
        service_startj = cur_time.unsqueeze(-1) + time2j  # [B, num_agents, N]

        # Calculate time to depot for each node
        depot_idx = self.td_state['depot_idx']  # [B, 1]

        # Get depot coordinates
        depot_coords = torch.gather(
            self.td_state['coords'],  # [B, N, 2]
            1,
            depot_idx.unsqueeze(-1).expand(-1, -1, 2)  # [B, 1, 2]
        )  # [B, 1, 2]

        # Calculate distance from each node to depot
        # node_coords [B, N, 2] -> [B, 1, N, 2]
        node_coords_expanded = self.td_state['coords'].unsqueeze(1)  # [B, 1, N, 2]
        # depot_coords [B, 1, 2] -> [B, 1, 1, 2]
        depot_coords_expanded = depot_coords.unsqueeze(2)  # [B, 1, 1, 2]

        diff_to_depot = node_coords_expanded - depot_coords_expanded  # [B, 1, N, 2]
        distance_to_depot = torch.sqrt((diff_to_depot ** 2).sum(dim=-1))  # [B, 1, N]
        time2depot = distance_to_depot / self.td_state['speed'].unsqueeze(-1)  # [B, 1, N]

        # Expand to all agents
        time2depot = time2depot.expand(-1, num_agents, -1)  # [B, num_agents, N]

        if hasattr(self, 'n_digits') and self.n_digits is not None:
            time2depot = torch.floor(self.n_digits * time2depot) / self.n_digits

        # Time constraint
        # Get service_time and end_time
        service_time = self.td_state.get('service_time', 0)
        service_time = service_time.unsqueeze(1)  # [B, 1, N]
        service_time = service_time.expand(-1, num_agents, -1)  # [B, num_agents, N]

        end_time = self.td_state['end_time']  # [B, 1]

        c_time = service_startj + service_time + time2depot <= end_time.unsqueeze(-1).unsqueeze(-1)  # [B, num_agents, N]
    ...
    def _update_state(self, action):
        ...
        time_update = arrivej + service_time
    ...
    def check_solution_validity(self):
        """
        Check if solution is valid according to TOP constraints.
        ...
        for ii in range(sorted_data.size(1)):
            ...
            # c1: must be able to finish service and return to depot before end_time
            assert torch.all(arrivej + service_time + time2depot <= self.td_state['end_time'].unsqueeze(-1)), "Cannot finish service and return to depot in time."
            ...
            curr_time = arrivej + service_time
            curr_node = next_node
