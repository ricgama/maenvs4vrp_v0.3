...
class Environment(AECEnv):
    ...
    Extends the CVRPTW environment by relaxing the hard time window constraint into a soft one.
    At each step, the agent chooses a customer to visit depending on the current location, the
    remaining capacity, and time window feasibility. Vehicles may arrive outside a customer's time
    window :math:`[e_i, l_i]` within a tolerance controlled by :math:`P_{max}` and :math:`W_{max}`,
    but incur early and late penalties proportional to the deviation. Early arrivals also incur a
    waiting penalty if the wait exceeds :math:`W_{max}`.
    ...
    Constraints:
        ...
        - service must begin within the extended window :math:`[e_i - P_{max}, l_i + P_{max}]`;
          violations outside this range are infeasible.
        - vehicles must arrive no earlier than :math:`e_i - P_{max} - W_{max}`.
        - early and late arrivals within the soft tolerance incur penalties.
    ...
    def reset(self,
        ...
        # Compute initial action masks so that the first sample_action call
        # already respects all constraints (arrive_limit, tw_high_limit, etc.)
        self._update_all_agents_feasibility()
    ...
    def _update_all_agents_feasibility(self):
        ...
        _mask = (
            self.td_state['nodes']['active_nodes_mask'].unsqueeze(1).expand(-1, self.num_agents, -1) *
        ...
        # All node coords: [B, num_nodes, 2]
        all_coords = self.td_state['coords']  # [B, num_nodes, 2]
        ...
        waitj = torch.clip(self.td_state['tw_low_limit'].unsqueeze(1) - arrivej, min=0)
        service_startj = arrivej + waitj

        # Time window constraints (broadcast node tensors over agents dim)
        c0 = arrivej >= self.td_state['arrive_limit'].unsqueeze(1)
        c1 = service_startj <= self.td_state['tw_high_limit'].unsqueeze(1)
        c2 = (service_startj +
        ...
        # Capacity constraint: [B, num_agents, num_nodes]
        c3 = (self.td_state['demands'].unsqueeze(1) <=
              self.td_state['agents']['cur_load'].unsqueeze(-1))
    ...
    def check_solution_validity(self):
        ...
        for ii in range(sorted_data.size(1)):
            ...
            # c0: can only arrive at or after arrive_limit
