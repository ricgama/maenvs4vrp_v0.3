...
class Environment(AECEnv):
    """
    General Multi-Task Vehicle Routing Problem (GMTVRP) environment.
    Extends the MTVRP environment with support for Mixed Backhaul (MB) and vehicle initial loads,
    covering up to 24 VRP variants. At each step, the agent chooses a customer to visit. The
    environment handles any combination of the following constraints:
    ...
    def _update_all_agents_feasibility(self):
        ...
        # precomputed time2depot/distance2depot from nodes: [B, N] -> [B, A, N]
        ...
        # Constraint 2: if closed route, can return to depot in time
        ...
        # Demand constraints — gmtvrp uses cur_linehaul_load / cur_backhaul_load
        # cur_linehaul_load: [B, A] -> [B, A, 1]
        # cur_backhaul_load: [B, A] -> [B, A, 1]
        ...
        # Backhaul class 1: unmixed, linehauls before backhauls
    ...
    def _post_process_mask(self, mask):
        ...
        # force_visit: agent at depot must leave if feasible non-depot nodes remain
        if self.force_visit:
