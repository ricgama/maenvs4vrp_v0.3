...
class Environment(AECEnv):
    ...
    At each step, the agent chooses a customer to visit depending on the current location, the
    remaining capacity, and time window feasibility. When the agent visits a customer, the remaining
    capacity is updated and time is advanced. Vehicles must arrive within each customer's time window;
    early arrivals wait until the window opens. If no feasible customer can be reached in time or
    capacity is insufficient, the agent must return to the depot.
    Unless the `force_visit` flag is set to True, in which case the agent must leave the depot if
    customers are available, vehicles are allowed to stay in the depot (being considered done).
    ...
    def _update_all_agents_feasibility(self):
        ...
        # Base mask from active nodes: [B, N] -> [B, 1, N] -> [B, num_agents, N]
        _mask = self.td_state['nodes']['active_nodes_mask'].clone()
        _mask = _mask.unsqueeze(1).expand(*batch_size, self.num_agents, self.num_nodes)

        # Get current locations for all agents: [B, num_agents, 2]
        cur_node_idx = self.td_state['agents']['cur_node_idx']  # [B, num_agents]
        loc = self.td_state['coords'].gather(1, cur_node_idx[:, :, None].expand(-1, -1, 2))  # [B, num_agents, 2]

        # Current time for all agents: [B, num_agents]
        ptime = self.td_state['agents']['cur_time'].clone()

        # Calculate distances and times to all nodes
        # coords: [B, N, 2], loc: [B, num_agents, 2]
        # distance2j: [B, num_agents, N]
        coords_expanded = self.td_state['coords'].unsqueeze(1).expand(*batch_size, self.num_agents, -1, -1)
        loc_expanded = loc.unsqueeze(2).expand(-1, -1, self.num_nodes, -1)
        distance2j = get_distance(loc_expanded, coords_expanded)
        ...
        arrivej = ptime.unsqueeze(-1) + time2j  # [B, num_agents, N]
        waitj = torch.clip(self.td_state['tw_low'].unsqueeze(1) - arrivej, min=0)  # [B, num_agents, N]
        service_startj = arrivej + waitj  # [B, num_agents, N]

        c1 = service_startj <= self.td_state['tw_high'].unsqueeze(1)  # [B, num_agents, N]
        c2 = service_startj + self.td_state['service_time'].unsqueeze(1) + self.td_state['time2depot'].unsqueeze(1) <= self.td_state['end_time'].unsqueeze(-1).unsqueeze(-1)  # [B, num_agents, N]

        # Capacity constraints
        c3 = self.td_state['demands'].unsqueeze(1) <= self.td_state['agents']['cur_load'].unsqueeze(-1)  # [B, num_agents, N]
