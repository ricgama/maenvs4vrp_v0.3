...
class Environment(AECEnv):
    """
    Prize Collecting Capacitated Vehicle Routing Problem (PCVRP) environment.
    ...
    def _update_curr_agent_feasibility(self):
        ...
        # constraint: agent can serve node and return to depot before end_time
        ...
        c2 = arrivej + self.td_state['service_time'] + self.td_state['time2depot'] <= self.td_state['end_time'].unsqueeze(-1)
    ...
    def _update_all_agents_feasibility(self):
        ...
        # c2: agent can serve node and return to depot before end_time
        c2 = (arrivej +
    ...
    def _post_process_mask(self, mask):
        ...
        No force_visit for PCVRP (profit maximization, visits are optional).
        """
    ...
    def check_solution_validity(self):
        ...
        for ii in range(sorted_data.size(1)):
            ...
            # c2: agent must return to depot before end_time
            at_depot = (next_node == 0)
            assert torch.all(~at_depot | (curr_time <= self.td_state['end_time'])), "Agent returned to depot after end_time."
            curr_node = next_node
