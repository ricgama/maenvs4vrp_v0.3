...
class Environment(AECEnv):
    ...
    def _update_all_agents_feasibility(self):
        ...
        """
        # Base mask: active_nodes_mask * agents['action_mask'] — same as _update_curr_agent_feasibility
        # [B, num_nodes] -> [B, num_agents, num_nodes]
        ...
        # c3: SDVRPTW — if cur_load <= 0, agent can ONLY return to depot (vectorized)
        # cur_load: [B, num_agents] -> empty_agents: [B, num_agents]
        empty_agents = self.td_state['agents']['cur_load'].le(0)               # [B, num_agents]

        # c3: start all True, zero out rows where agent is empty [B, num_agents, num_nodes]
        c3 = ~empty_agents.unsqueeze(-1).expand(-1, -1, self.num_nodes)        # [B, num_agents, num_nodes]

        # re-open depot for empty agents via scatter
        # depot_idx: [B, 1] -> [B, num_agents, 1]
        depot_idx_exp = self.td_state['depot_idx'].unsqueeze(1).expand(-1, self.num_agents, 1)
        # for empty agents, depot is True; for non-empty agents, keep c3 value (True)
        depot_val = torch.ones(*self.batch_size, self.num_agents, 1, dtype=torch.bool, device=self.device)
        c3.scatter_(2, depot_idx_exp, depot_val)                               # depot always open
    ...
    def _post_process_mask(self, mask):
        ...
        depot_idx_exp = self.td_state['depot_idx'].unsqueeze(1).expand(*batch_size, self.num_agents, 1)
