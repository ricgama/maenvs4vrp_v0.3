...
class Environment(AECEnv):
    """
    Dynamic Vehicle Routing Problem with Time Windows (DVRPTW) environment.
    Extends the CVRPTW environment with dynamic customer arrivals. Customers become available over
    time (each has an `appear_time`), so not all customers are known at the start of the episode.
    At each step, the agent chooses a customer to visit from those that have already appeared and
    are still feasible given the current time, capacity, and time windows.
    ...
    def _update_all_agents_feasibility(self):
        ...
        """
        # cust_mask: appear_time [B, num_nodes], cur_time [B, num_agents]
        ...
        # same constraints as _update_curr_agent_feasibility — note: tw_low/tw_high (not tw_low_limit)
    ...
    def _post_process_mask(self, mask):
        ...
        # Open depot for agent 0 only in done batch rows (batch consistency)
        ...
        # force_visit: agents at depot must leave if unvisited nodes remain
        if self.force_visit:
            ...
            depot_idx_exp = depot_idx.unsqueeze(1).expand(*batch_size, self.num_agents, 1)
