...
class Environment(AECEnv):
    ...
    At each step, the agent chooses a node to visit (either a pickup or delivery node). Each request
    consists of a paired pickup and delivery location. Vehicles must collect the load at the pickup node
    before delivering it to the corresponding delivery node. All nodes have time windows within which
    service must begin; early arrivals wait.
    Unless the `force_visit` flag is set to True, in which case the agent must leave the depot if
    requests are available, vehicles are allowed to stay in the depot (being considered done).
    ...
    Finish Condition:
        - all requests have been served and every vehicle has returned to the depot.
    """
    def __init__(self,
    ...
    def reset(self,
        ...
        self.td_state['agents'] =  TensorDict(
                                    source={'capacity': self.td_state['capacity'],
                                            ...
                                            'action_mask': feasible_nodes.unsqueeze(1).repeat((1, self.num_agents, 1)),
    ...
    def _update_all_agents_feasibility(self):
        ...
        # c1, c2: time window constraints — same as _update_curr_agent_feasibility
        ...
        # c3: capacity — PDPTW: cur_load + demands <= capacity
