...
INSTANCES_PATH = 'pdptw/data/generated'
DATA_PATH = './pdptw/data/generated'
...
class InstanceGenerator(InstanceBuilder):
    ...
    def random_generate_instance(self,
        ...
        delivery_idx = torch.arange(num_services+1, self.num_nodes, device=self.device).repeat(self.batch_size.numel(),1)

        instance['pickup_idx'] = torch.zeros((*self.batch_size, self.num_nodes), dtype = torch.long, device=self.device)
        instance['delivery_idx'] = torch.zeros((*self.batch_size, self.num_nodes), dtype = torch.long, device=self.device)
        ...
        instance['is_pickup'] = torch.zeros((*self.batch_size, self.num_nodes), dtype=torch.bool, device=self.device)
        instance['is_pickup'].scatter_(1, pickup_idx, True)

        instance['is_delivery'] = torch.zeros((*self.batch_size, self.num_nodes), dtype=torch.bool, device=self.device)
        ...
        instance['demands'] = torch.zeros((*self.batch_size, self.num_nodes), dtype = torch.float, device=self.device)
