...
# Resolve paths from this file so imports work regardless of cwd.
...
from parco_policy_net import PolicyNet
...
def reinforce_shared_baseline_loss(logprobs, rewards, entropy, step_mask, args, reduction = 'mean'):
    ...
    loss = -(advantages.unsqueeze(0) * logprobs_sum + args.ent_coef * entropy_sum) * step_mask
    loss = loss.sum(0)
...
def train_epoch(args, env, policy, optim, ep, writer):
    ...
    with trange(args.iter_count, desc = "Ep.#{: >3d}/{: <3d}".format(ep+1, args.epoch_count)) as progress:
        for episode in progress:
            ...
            td = env.reset_observe(num_agents=args.num_agents,
                           num_nodes=args.num_nodes,
                           seed=args.seed+ep*args.iter_count+episode,
                           force_visit=args.force_visit,
                           sample_type='augment',
                           n_augment=args.n_augment,
                           obs_list=['nodes_static', 'all_agents', 'global', 'agents_cur_node_idx', 'agents_action_mask', 'active_agents_mask'])
            ...
            policy.make_cache_(nodes_obs=node_stat_obs, agents_obs=all_agents_obs)
            
            while not td["done"].all():

                # rollover the observation
                obs = td['observations']
                agents_obs = obs.get('all_agents_obs', None)
                global_obs = obs.get('global_obs', None)
                node_dyn_obs = obs.get('nodes_dynamic_obs', None)
                action_mask  = obs.get('agents_action_mask', None)
                active_agents_mask = obs.get('active_agents_mask', None)
                cur_node_idx = obs.get('agents_cur_node_idx', None)

               # get action from the policy
                action, logprobs, ent = policy.get_centralized_action_and_logs(nodes_dyn_obs=node_dyn_obs,
                                                                          all_agents_obs=agents_obs,
                                                                          global_obs=global_obs,
                                                                          cur_node_idx=cur_node_idx,
                                                                          mask=action_mask,
                                                                          active_agents_mask=active_agents_mask )

                td['next_actions'] = action
                
                # execute the environment and log data

                td = env.step_all_observe(
                    td, 
                    obs_list=['nodes_static', 'all_agents', 'global', 'agents_cur_node_idx', 'agents_action_mask', 'active_agents_mask'],
                )
...
def ct_test_epoch(args, test_env, policy, ep, writer):
    policy.eval()
    ...
    with torch.no_grad():
        for instance_name in test_env.inst_generator.set_of_instances:
            ...
            td = test_env.reset_observe(num_agents=args.num_agents,
                                num_nodes=args.num_nodes,
                                force_visit=args.force_visit,
                                sample_type='saved',
            ...
            policy.make_cache_(nodes_obs=node_stat_obs,
                               agents_obs=td['observations'].get('all_agents_obs', None))
            ...
            while not td["done"].all():
                ...
                node_dyn_obs = obs.get('nodes_dynamic_obs', None)
                action_mask  = obs.get('agents_action_mask', None)
                cur_node_idx = obs['agents_cur_node_idx']

                # get action from the agent
                action, _, _ = policy.get_centralized_action_and_logs(nodes_dyn_obs=node_dyn_obs,
                                                                 all_agents_obs=agents_obs,
                                                                 global_obs=global_obs,
                                                                 cur_node_idx=cur_node_idx,
                                                                 mask=action_mask,
        ...
        writer.add_scalar("eval_ct/episodic_return", total_reward, ep)
        writer.add_scalar("eval_ct/episodic_not_visited_nodes", not_visited_nodes, ep)
        writer.add_scalar("eval_ct/episodic_number_used_agents", number_used_agents, ep)
...
def train(args, writer):
    ...
    observations_module_name = f'parallel_environments.{args.vrp_env}.observations'
    observations = importlib.import_module(observations_module_name).Observations(feature_list)

    generator_module_name = f'parallel_environments.{args.vrp_env}.instances_generator'
    generator = importlib.import_module(generator_module_name).InstanceGenerator(device=args.device)

    environment_module_name = f'parallel_environments.{args.vrp_env}.env'
    environment_module = importlib.import_module(environment_module_name)

    env_agent_reward_module_name = f'parallel_environments.{args.vrp_env}.env_agent_reward'
    ...
    agents_obs_dim = env.obs_builder.get_all_agents_feat_dim()
    global_obs_dim = env.obs_builder.get_global_feat_dim()

    """ ALGO LOGIC: EPISODE STORAGE"""
    start_time = time.time()

    print("Initializing PARCO model...")
    ...
    for ep in range(0, args.epoch_count):
        ...
        scheduler.step()
        test_metrics = ct_test_epoch(args, eval_env, policy, ep, writer)
        ...
        writer.add_scalar("eval_ct/best_model_lb_total_reward", best_lb_total_return, ep)
...
def parse_args():
    ...
    parser.add_argument("--vrp_env", type=str, default="top", help="select the vrp environment to train on")
...
def main(args):
    ...
    if not args.debug:
        wandb.init(project="Coop", entity="mustelideos",
